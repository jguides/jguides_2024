...
class DecodeCovFRVecBase(ComputedBase):
    ...
    def get_concat_metric_df(self, keys, debug_mode=False):
        ...
        # Otherwise do using parallelization
            ...
            df_list = pool.map(_get_dfs, keys)
            pool.close()
            pool.join()
    ...
    @staticmethod
    def _save_dir():
        return "/nimbus/jguidera/decoding/concat_df"

    def _concat_metric_df_file_name_base(self):
        return f"concat_df_{self.table_name}_"

    def save_concat_metric_df(self, keys):

        import random
        rand_int = random.randint(0, 10000)

        file_name = f"{self._concat_metric_df_file_name_base()}{rand_int}"

        # Concatenate table entries and save out dataframe. Raise error if file already exists.
        concat_df = self.get_concat_metric_df(keys)
        pickle_file(concat_df, file_name, save_dir=self._save_dir(), overwrite=False)

        return file_name

    def load_concat_metric_df(self, file_name):
        expected_text = self._concat_metric_df_file_name_base()
        if file_name[:len(expected_text)] != expected_text:
            raise Exception(f"File must start with {expected_text}")
        return unpickle_file(file_name, self._save_dir())

    def delete_concat_metric_df(self, file_name):
        expected_text = self._concat_metric_df_file_name_base()
        if file_name[:len(expected_text)] != expected_text:
            raise Exception(f"File must start with {expected_text}")
